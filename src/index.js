...
const { registerCommands } = require(.discordregisterCommands);
const {
  DISCORD_TOKEN,
  GUILD_ID,
...
const { partyBoardEmbed, partyBoardComponents } = require(.partyui);
const { nicknameBoardComponents } = require(.featuresnicknameui);
const { handleNickname } = require(.featuresnicknamehandler);
const { handleParty } = require(.partyhandler);

 ✅ 카카오 TXT importrebuildreset
const {
  handleKakaoImport,
  handleKakaoRebuild,
  handleKakaoClear,
} = require(.featureskakaohandler);
...
function csvEscape(value) {
  ...
  if ([,n].test(s)) return `${s.replace(g, '')}`;
  return s;
}
...
async function sendWelcomeEmbed(guild, { title, member, color = 0x5865f2 }) {
  ...
  const embed = new EmbedBuilder()
    .setColor(color)
    .setDescription(`## ${title}nn${mainLine}n${roleLine}`);
...
client.once(clientReady, async () = {
  ...
  if (ENABLE_PARTY) {
    ...
    if (board.isTextBased()) {
      await ensurePinnedMessage(board, DDGpartyboardv1, () = ({
  ...
  if (ENABLE_NICK && NICK_HELP_CHANNEL_ID) {
    ...
    if (nickCh.isTextBased()) {
      await ensurePinnedMessage(nickCh, DDGnickboardv1, () = ({
        embeds [
          {
            title 🪪 닉네임 설정,
            description 아래 버튼으로 서버 별명을 변경합니다.,
            footer { text DDGnickboardv1 },
          },
        ],
...
client.on(interactionCreate, async (interaction) = {
  try {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === included_members_export) {
      ...
      await interaction.editReply({
        content `✅ included 멤버 ${rows.length}명 CSV 내보내기 완료 (제외 봇외출부계)`,
        files [file],
      }).catch(() = {});
      return;
    }
